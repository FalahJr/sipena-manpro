     ...
     @endif
        ...
        <div class="row">
          ...
          <form action="{{url('admin/kelas/update')}}" method="POST">
            {{ csrf_field() }}
          <tr>
            <td>Kategori Keuangan <span style="color:red;">*</span></td>
            <td>
            <select class="form-control form-control-sm inputtext @if($errors->has('keuangan_kategori_id')) is-invalid @endif" value="{{$data->keuangan_kategori_id}}" name="keuangan_kategori_id">
                  <?php foreach($kategori as $list_kategori){ ?>

                  <option value="<?= $list_kategori->id ?>">
                    <?= $list_kategori->nama ?>
                  </option>
                  <?php }?>
                </select>
